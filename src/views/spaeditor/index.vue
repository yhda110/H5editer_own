  ...
  export default {
    ...
    methods: {
      dialogSave () {
        ...
        return this.$router.replace('spaList')
      },
      getPicList (id) {
        this.$store.dispatch('getPicListByThemeId', id)
      },
      uploadImage (data) {
      ...
      deploy () {
        console.log('deploy', tools.vue2json(this.$store.state.editor.editorTheme))
    ...
    mounted () {
      this.itemId = this.$store.state.editor.editorTheme.id || this.$route.query.itemId
